#include "QuickTestCPP/QuickTestCPP.h"
...
int main()
{
    ...
    TestRunner::GetRunner()->AddTest(
        "Unpacker",
        "Can unpack two channels packed",
        []() {
            Unpacker unpacker;
            ...
            std::vector<int8_t> truth1 = {
                3, -3, 1, 3};

            std::vector<int8_t> truth2 = {
               -1, 1, -3, -1
               };
            ...
            for (unsigned i = 0; i < 4; i++)
            {
                ...
                {
                    std::cout << "0 " << i << " : " << (signed)truth1[i] <<" "<<(signed)outputs[0][i]<< std::endl;
                    return 0;
                }
            }
            ...
            for (unsigned i = 0; i < 4; i++)
            {
                ...
                {
                    std::cout << "1 " << i << " : " << (signed)truth2[i] <<" "<< (signed)outputs[1][i]<<std::endl;
                    return 0;
                }
            }
    ...
    TestRunner::GetRunner()->AddTest(
        "Unpacker",
        "Correct output buffer size for 2chn 2bit IQ",
        []() {
            Unpacker unpacker;

            if (unpacker.GetChanSamplesPerByte() != 1.0)
            {
                std::cerr << "Incorrect samples per byte" << unpacker.GetChanSamplesPerByte() << std::endl;
                return 0;
            }
    ...
    TestRunner::GetRunner()->AddTest(
        "Unpacker",
        "Correct output buffer size for 4chn 2bit IQ",
        []() {
    ...
    TestRunner::GetRunner()->AddTest(
        "Unpacker",
        "Correct output buffer size for 1chn 2bit IQ",
        []() {
            std::vector<int> map = {1, 3, -3, -1};
