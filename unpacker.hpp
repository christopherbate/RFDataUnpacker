...
class Unpacker
{
  public:
     ...
     */
    Unpacker(std::vector<int> &map, unsigned int numChannels, unsigned int numBits,bool complex) : m_map(map), m_numChannels(numChannels), m_numBits(numBits),m_isComplex(complex)
    {        
        ResetIdicies();
    }
    ...
    void ResetIdicies()
    {
        m_componentIdx = m_channelIdx = 0;
    }

    double GetChanSamplesPerByte()
    {
        return ((double)(8 / m_numBits) / (double)(m_isComplex ? 2 : 1) / (double)m_numChannels) ;
    }
     ...
     * Unpack
     */
    void Unpack(std::vector<uint8_t> &inputBuffer, std::vector<std::vector<int8_t> > &outputBuffers)
    {        
        unsigned int outputIdx = 0;        

        // Verify output Size.
        unsigned int outputSize = GetOutputBufferSize(inputBuffer.size());
        for(auto iter = outputBuffers.begin(); iter != outputBuffers.end(); iter++)
        {
        ...
        for (unsigned int i = 0; i < inputBuffer.size(); i++)
        {
            ...
            for (unsigned int j = 0; j < 8 / m_numBits; j++)
            {
                ...
                outputBuffers[m_channelIdx][outputIdx+m_componentIdx]=(samplePoint);
