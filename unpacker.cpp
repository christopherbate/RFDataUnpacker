...
int main()
{
    ...
    try
    {
        ...
        {
            throw std::runtime_error("Must specify the same number of channels as output filenames");
        }
        ...
        for(auto iter = channelFilenames.begin(); iter!=channelFilenames.end();iter++)
        {
            outFiles[outIdx].open( (*iter).c_str(), std::ofstream::binary | std::ofstream::out);
            if(!outFiles[outIdx].is_open()){
                throw std::runtime_error("Could not open output file.");
            }
            outputBuffers[outIdx].resize(unpacker.GetOutputBufferSize( bufferSize ));
            outIdx++;
        }        
        ...
        if(!infile.is_open()){
            throw std::runtime_error("Error opening file. Does not exist or cannot be opened.");
        }
        ...
        while (!infile.eof())   
        {
            ...
            {
                std::cout<< total<<" bytes processed, "<< (double)total/(double)fileSize <<" percent complete."<<std::endl;
                lastPrint = total;
            }            
        }
